...
<div style="background-color: rgb(255, 255, 255)" class="articles container pb-3 mt-5">
    ...
    <div class="row">
      ...
      <div class="mt-3 col-lg-4 col-md-6 col-sm-12 ">
        <div style="min-height: 100%" class="card">
          <img src="{{asset('/img')}}/{{$tuto->image}}" class="card-img-top" alt="...">
            <div style="background-color: rgb(246, 246, 246)" class="card-body">
              ...
              <p style="margin-top: -0.5em" class="card-text">{!!substr(html_entity_decode($tuto->description)0,30)."..."!!}</p>
              <p style="margin-top: -0.8em" class="card-date">Publié le: {!!html_entity_decode($tuto->date_publication)!!}</p>
              <p style="margin-top: -0.8em" class="card-auteur">Par: <strong>{!!html_entity_decode($tuto->auteur)!!}</strong></p>
              <a style="margin-top: -0.5em; border: none; background-color:#211061" href="/article/{{$tuto->id}}" class="btn btn-primary">Lire plus</a>
            </div>
        </div>
      </div> 
      @endforeach   
  ...
  @else
    <div class="row">
      <div class="col-lg-7">
        <h2 style="color: #211061; border-radius: 4px" class="pb-2 border text-center">Aucun tutoriel disponible pour le moment</h2>
      </div>
    </div>
  @endif
</div>
